...
const withdrawShipment = async (shipmentId, reason) => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/shipments/${shipmentId}/withdraw`, {
    method: 'POST',
    headers: { 
...
const downloadAudit = async (shipmentId) => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/shipments/${shipmentId}/audit-export`, {
...
const [filters, setFilters] = useState({ status: '', commodity: '', destination: 'NL', search: '' }); // Default NL for Netherlands corridor
  ...
  const loadSummary = useCallback(async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/shipments/summary`, {
        headers: { Authorization: `Bearer ${getToken()}` }
      });
  ...
  const connectWS = useCallback(() => {
    ...
    const wsUrl = `${process.env.NEXT_PUBLIC_WS_URL || 'ws://localhost:3001'}/ws/shipments?token=${token}`;
    ...
    newWs.onclose = () => {
      ...
      pollIntervalRef.current = setInterval(loadShipments, 30000);
            ...
            {shipments.length === 0 ? (
              <tr>
                <td colSpan={9} className="text-center py-8 text-muted-foreground">
                  No shipments for selected corridor. Create your first shipment to see live data.
                </td>
              </tr>
            ) : (
